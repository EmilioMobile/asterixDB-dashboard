...
export class SQLService {
    ...
    */
    executeSQLQuery(query: string): Observable<AsterixDBQueryMessage> {
        ...
        let observer = this.http.post(apiUrl, {statement: query,
            'logical-plan': true,
            'optimized-logical-plan': true }, options)
            .map((response: Response) => {
              //console.log(response);
              //let kk = Object.create(response);
            // let kk = JSON.parse(JSON.stringify(response))
            // let index = this.dataService.push(kk);
            // response['results'] = [];
            //  console.log(response);
              return response;
            })
